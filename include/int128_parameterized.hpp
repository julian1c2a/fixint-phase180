...
namespace nstd
{
    ...
    class int128_param_t
    {
        using self_type = int128_param_t<Sign, Form>;

    public:
        // ... (otros miembros previos) ...

        // ========================================================================
        // Increment/Decrement Operators y next/previous
        // ========================================================================

        /// @brief Pre-increment (++x)
        ...
        /// @brief Returns next value (copy, value semantics)
        [[nodiscard]] constexpr int128_param_t next() const noexcept
        {
        ...
        /// @brief Returns previous value (copy, value semantics)
        [[nodiscard]] constexpr int128_param_t previous() const noexcept
        {
            int128_param_t result{*this};
            --result;
            return result;
        ...
        // ========================================================================
        // División y módulo simultáneos (divmod)
        // ========================================================================

        /**
         * @brief División y módulo simultáneos (divmod)
         *
         * Calcula el cociente y el resto de la división entera de *this por other.
         * Para representaciones TC y MS, usa aritmética estándar; para EK, convierte a TC.
         *
         * @param other Divisor
         * @return std::pair<quociente, resto>
         */
        [[nodiscard]] constexpr std::pair<self_type, self_type>
        divmod(const self_type &other) const noexcept
        {
            self_type quotient{0};
            self_type remainder{*this};
            const bool neg_dividend = self_type::is_signed && remainder.is_negative();
            const bool neg_divisor = self_type::is_signed && other.is_negative();
            self_type divisor_abs = neg_divisor ? -other : other;
            remainder = neg_dividend ? -remainder : remainder;
            if (divisor_abs.is_zero())
            {
                return {self_type{0}, *this};
            }
        ...
        // Métodos auxiliares públicos requeridos por divmod
        ...
        {
            if constexpr (self_type::is_twos_complement)
            {
                return (this->data[1] >> 63) != 0;
            }
            else if constexpr (self_type::is_magnitude_sign)
            {
                return (this->data[1] >> 63) != 0;
            }
            else if constexpr (self_type::is_excess_k)
            {
                constexpr std::uint64_t bias_high = (1ULL << 62);
                return this->data[1] < bias_high;
            }
            else
            {
                return false;
        ...
        // (Eliminados duplicados de operadores ++, --, next, previous)
        {
        public:
            // ========================================================================
            ...
            // Enforce valid combinations: unsigned solo permite binnat, signed no permite binnat
            static_assert(
