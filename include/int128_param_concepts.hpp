...
namespace nstd
{
    ...
    namespace detail
    {
         ...
         */
        template <typename T>
        struct is_signed_int128_impl
            : std::bool_constant<is_int128_tc_impl<T>::value || is_int128_ms_impl<T>::value ||
     ...
     */
    template <typename T>
    inline constexpr bool is_128bit_type_v = detail::is_int128_param_impl<std::remove_cv_t<T>>::value;
     ...
     */
    template <typename T>
    concept accumulative_type = std::default_initializable<T> && requires(T a, T b) {
    ...
    concept int128_predicate = int128_type<T> && requires(Pred pred, T value) {
    ...
    concept int128_iterator =
        std::input_or_output_iterator<Iter> && int128_type<typename std::iterator_traits<Iter>::value_type>;
