...
| **La división** | 🔸 **abierta, pero P2.10 cerrada**. Knuth D en su capa medible (`div_kernels.hpp`), `divq` en línea **1,28×–1,39×**, Möller–Granlund **2/1** (~84 → ~17 ciclos/limbo) y **3/2** (hasta **3,3×** con divisor corto). Queda **P2.11** (Burnikel–Ziegler) |
...
| **P2.11** | **Burnikel–Ziegler**: la división pasa de Θ(N²) a Θ(M(N)·log N) | ⚠️ **El umbral esperado era falso, y por veinte veces.** Medido (`benchmark_techo_division`), la división `(N, n=N/2)` cuesta **1,6×–2,8×** una multiplicación de `N/2` hasta N=1024 — o sea **ya dentro del techo de 2–4× de GMP**, donde B-Z no tiene qué recoger. Sólo lo pasa en **N=2048 (4,9×)**, cuando la multiplicación entra en Toom-3. Decisión pendiente: es trabajo grande para el extremo del rango |
