...
static void test_higher_arith_int(const char *tag)
{
    ...
    // MIGRADO el 5 sep 2026 (P1.3, ADR-009): las checked_* devuelven el propio
    // tipo con politica checked, no std::optional. `.has_value()` pasa a
    // `.valid()`, y el valor esta siempre.
    using C = decltype(nstd::con_comprobacion(I{}));
    ...
    // checked_add (signed)
    {
        ...
        TEST("checked_add(3,-4)==-1", nstd::checked_add(I{3}, I{-4}) == nstd::con_comprobacion(I{-1}));
        TEST("checked_add(max,min)==-1", nstd::checked_add(mx, mn) == nstd::con_comprobacion(I{-1}));
        TEST("checked_add(max,1) marca", !nstd::checked_add(mx, pos_one).valid());
        TEST("checked_add(min,-1) marca", !nstd::checked_add(mn, neg_one).valid());
    }
    ...
    // checked_sub (signed)
    {
        ...
        I pos_one{std::int64_t{1}};
        I neg_one{std::int64_t{-1}};
        TEST("checked_sub(5,3)==2", nstd::checked_sub(I{5}, I{3}) == nstd::con_comprobacion(I{2}));
        TEST("checked_sub(3,5)==-2", nstd::checked_sub(I{3}, I{5}) == nstd::con_comprobacion(I{-2}));
        TEST("checked_sub(min,1) marca", !nstd::checked_sub(mn, pos_one).valid());
        TEST("checked_sub(max,-1) marca", !nstd::checked_sub(mx, neg_one).valid());
    }
    ...
    // checked_mul (signed)
    {
        ...
        I neg_one{std::int64_t{-1}};
        I two{std::int64_t{2}};
        TEST("checked_mul(3,4)==12", nstd::checked_mul(I{3}, I{4}) == nstd::con_comprobacion(I{12}));
        TEST("checked_mul(-1,-1)==1", nstd::checked_mul(neg_one, neg_one) == nstd::con_comprobacion(I{1}));
        TEST("checked_mul(max,2) marca", !nstd::checked_mul(mx, two).valid());
        TEST("checked_mul(0,max)==0", nstd::checked_mul(I{}, mx) == nstd::con_comprobacion(I{}));
    }
}
