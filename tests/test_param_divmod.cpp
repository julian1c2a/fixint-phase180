...
static void test_operators_unsigned()
{
    ...
    {
        uint128_t a{0, 128}, b{0, 8};
        a /= b;
        TEST("128 /= 8  = 16", a.low() == 16);
    }
    {
        uint128_t a{0, 42}, b{0, 6};
        a /= b;
        TEST("42  /= 6  = 7", a.low() == 7);
    }
    {
        uint128_t a{0, 12345}, b{0, 1};
        a /= b;
        TEST("n  /= 1   = n", a.low() == 12345);
    }
    ...
    {
        uint128_t a{0, 20}, b{0, 5};
        a %= b;
        TEST("20 %= 5 = 0", a.low() == 0);
    }
    {
        uint128_t a{0, 19}, b{0, 5};
        a %= b;
        TEST("19 %= 5 = 4", a.low() == 4);
    }
    ...
    // edge cases
    {
        uint128_t a{0, 42}, b{0, 42};
        TEST("n / n = 1", (a / b).low() == 1);
    }
    {
        uint128_t a{0, 42}, b{0, 42};
        TEST("n % n = 0", (a % b).low() == 0);
    }
    {
        uint128_t a{0, 3}, b{0, 10};
        TEST("3 / 10 = 0, 3 % 10 = 3", (a / b).low() == 0 && (a % b).low() == 3);
    }
...
static void test_operators_signed_tc()
{
    ...
    {
        int128_tc_t a{0, 20}, b{0, 4};
        a /= b;
        TEST("(+20)/=(+4)=+5", a.low() == 5 && !a.is_negative());
    }
    {
        int128_tc_t a{static_cast<int64_t>(-20)}, b{0, 4};
        a /= b;
        TEST("(-20)/=(+4) is negative", a.is_negative());
    }
    {
        int128_tc_t a{0, 20}, b{static_cast<int64_t>(-4)};
        a /= b;
        TEST("(+20)/=(-4) is negative", a.is_negative());
    }
    {
        int128_tc_t a{static_cast<int64_t>(-20)}, b{static_cast<int64_t>(-4)};
        a /= b;
        TEST("(-20)/=(-4)=+5", a.low() == 5 && !a.is_negative());
    }
    {
        int128_tc_t a{0, 17}, b{0, 5};
        a %= b;
...
static void test_divmod_levels()
{
    ...
    // Level 1: power-of-2 → shift
    {
        ...
        TEST("2^127 / 2 = 2^126 r=0", q.low() == 0x4000000000000000ULL && r.is_zero());
    }
    {
...
static void test_knuth_correctness()
{
    ...
    }
    {
        ...
        TEST("64/64: (2^64-1)/(2^32-1) = 2^32+1 r=0", q.low() == 0x100000001ull && r.is_zero());
    }
    ...
    }
    {
        ...
        auto [q, r] = a.divmod(b);
        TEST("128/128: (2^128-1)/2^127 = 1 r=(2^127-1)", q.low() == 1 && q.high() == 0 &&
    ...
    }
    {
        ...
        auto [q, r] = a.divmod(b);
        TEST("128/128: near-equal q=1 r=4", q.low() == 1 && q.high() == 0 && r.low() == 4 && r.high() == 0);
    }
    {
        uint128_t a{0xFFFFFFFFFFFFFFFFull, 0xFFFFFFFFFFFFFFFFull};
...
static void test_knuth_vs_binary()
{
    ...
    const Case cases[] = {
        ...
        {0xABCDEF0123456789ull, 0x1234567890ABCDEFull, 0x1234567890ABCDEFull, 0xABCDEF0123456789ull,
         "large/large"},
    ...
    for (const auto &tc : cases)
    {
        ...
        if (qk == qb && rk == rb)
        {
            ...
            std::cout << "[FAIL] mismatch on " << tc.name << "\n";
    }
...
#define SWEEP_DIV(D_VAL)                                                                             \
    ...
    {                                                                                                \
        ...
        const bool ok{sweep_unary([](const uint128_t &n) { return n.template div<D_VAL>(); },        \
                                  [&](const uint128_t &n) { return n / d128; }, "div<" #D_VAL ">")}; \
        ++total;                                                                                     \
        if (ok)                                                                                      \
            ++passed_local;                                                                          \
    } while (false)
...
#define SWEEP_MOD(D_VAL)                                                                             \
    ...
    {                                                                                                \
        const auto d128{uint128_t{D_VAL}};                                                           \
        const bool ok{sweep_unary([](const uint128_t &n) { return n.template mod<D_VAL>(); },        \
                                  [&](const uint128_t &n) { return n % d128; }, "mod<" #D_VAL ">")}; \
        ++total;                                                                                     \
        if (ok)                                                                                      \
            ++passed_local;                                                                          \
    } while (false)
...
#define SWEEP_DIVMOD(D_VAL)                                                                   \
    ...
    {                                                                                         \
        ...
        const bool ok{sweep_unary(                                                            \
            ...
            [](const uint128_t &) { return true; }, "divmod_const<" #D_VAL "> consistency")}; \
        ++total;                                                                              \
        if (ok)                                                                               \
            ++passed_local;                                                                   \
    } while (false)
...
static void test_gm_algorithm_api()
{
    ...
    // fast_div10 / fast_mod10 sweeps
    {
        ...
        ok &= sweep_unary([](const uint128_t &n) { return fast_div10(n); },
                          [&](const uint128_t &n) { return n / ten; }, "fast_div10_matches_standard");
    ...
    // divmod10 consistency
    {
        ...
        const bool ok{sweep_unary(
            ...
            [](const uint128_t &) { return true; }, "divmod10 consistency q*10+r==n")};
        ++total;
        if (ok)
            ++passed_local;
    }
    ...
    {
        const bool ok{sweep_binary([](const uint128_t &a, const uint128_t &b) { return mulhi_128(a, b); },
    ...
    const auto test_one_divisor = [&](uint64_t D, uint128_t (*fdiv)(const uint128_t &),
                                      ...
                                      const char *label)
    {
        ...
        ok &= sweep_unary([=](const uint128_t &n) { return fdiv(n); },
                          [&](const uint128_t &n) { return n / d128; }, label);
        ok &= sweep_unary(
            [=, &d128](const uint128_t &n)
            {
    ...
    // 10^19 (special API: remainder is uint64_t)
    {
        ...
        ok &= sweep_unary([&](const uint128_t &n) { return fast_div_1e19(n); },
                          [&](const uint128_t &n) { return n / e19; }, "fast_div_1e19");
        ok &= sweep_unary(
            [&](const uint128_t &n)
            {
...
int main()
{
    ...
    if (g_failed == 0)
        ...
        std::cout << "[FAIL] " << g_failed << " TESTS FAILED\n";
