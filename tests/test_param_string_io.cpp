...
static void test_to_string_bases()
{
    ...
    {
        const uint128_t v{0, 0b1010};
        TEST("to_string bin 0b1010", v.to_string(2) == "1010");
    }
    ...
    {
        const uint128_t v{0, 0777};
        TEST("to_string oct 0777", v.to_string(8) == "777");
    }
    ...
    {
        const uint128_t v{0, 0xFF};
        TEST("to_string hex 0xFF", v.to_string(16) == "FF");
    }
    {
    ...
    // All bases 2..36 produce non-empty strings
    {
        ...
        bool all_ok = true;
        for (int b = 2; b <= 36; ++b)
            all_ok = all_ok && !x.to_string(b).empty();
...
static void test_from_string()
{
    ...
    }
    {
        ...
        TEST("from_string hex DEADBEEFCAFEBABE", uint128_t::from_string("0xDEADBEEFCAFEBABE") == exp);
    }
    ...
    // Roundtrips
    {
        ...
        TEST("from_string roundtrip decimal", uint128_t::from_string(orig.to_string().c_str()) == orig);
    }
    {
...
static void test_parse_ct_safe()
{
    ...
    // Error: empty string
    {
        ...
        TEST("parse empty -> empty_string error", !r.success() && r.error == parse_error::empty_string);
    }

    // Error: null pointer
    {
        const auto r = int128_t::parse_ct_safe(nullptr);
        TEST("parse null -> null_pointer error", !r.success() && r.error == parse_error::null_pointer);
    }

    // Error: invalid character
    {
        const auto r = int128_t::parse_ct_safe("12@45");
        TEST("parse invalid_char error", !r.success() && r.error == parse_error::invalid_character);
    }

    // Separator '_' is skipped
    {
        const auto r = int128_t::parse_ct_safe("1_000_000");
        TEST("parse separators 1_000_000", r.success() && r.value == int128_t{1000000LL});
    }
