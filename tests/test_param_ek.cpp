...
static void test_addition()
{
...

    {
        ek_t r = ek(1);
        r += ek(1);
        TEST("add 1+1=2", r == ek(2));
    }
    {
        ek_t r = ek(1);
        r += ek(-1);
        TEST("add 1+(-1)=0", r == ek(0));
    }
    {
        ek_t r = ek(1000);
        r += ek(2000);
        TEST("add 1000+2000=3000", r == ek(3000));
    }
    {
        ek_t r = ek(-10);
        r += ek(-20);
        TEST("add (-10)+(-20)=-30", r == ek(-30));
    }

    {
        const ek_t a = ek(42), b = ek(17);
        TEST("add commutative a+b==b+a", a + b == b + a);
    }
    {
        const ek_t a = ek(100), b = ek(200);
        TEST("add operator+: 100+200=300", a + b == ek(300));
    }
    {
        const ek_t a = ek(100), z = ek(0);
        TEST("add identity: a+0==a", a + z == a);
    }
}
...
static void test_subtraction()
{
...

    {
        ek_t r = ek(5);
        r -= ek(3);
        TEST("sub 5-3=2", r == ek(2));
    }
    {
        ek_t r = ek(999);
        r -= ek(999);
        TEST("sub a-a=0", r == ek(0));
    }
    {
        ek_t r = ek(3);
        r -= ek(5);
        TEST("sub 3-5=-2", r == ek(-2));
    }
    {
        ek_t r = ek(-5);
        r -= ek(-3);
    ...
    {
        const ek_t a = ek(100), b = ek(200);
        TEST("sub operator-: 100-200=-100", a - b == ek(-100));
    }
    {
        const ek_t a = ek(100), z = ek(0);
        TEST("sub identity: a-0==a", a - z == a);
    }
}
...
static void test_roundtrip()
{
    ...
    {
        const ek_t a = ek(12345), b = ek(6789);
        TEST("(a+b)-b==a positive", (a + b) - b == a);
    }
    {
        const ek_t a = ek(10000), b = ek(3333);
        TEST("(a-b)+b==a positive", (a - b) + b == a);
    }
    {
        const ek_t a = ek(-42), b = ek(100);
        TEST("(a+b)-b==a negative a", (a + b) - b == a);
    }
}
...
static void test_inc_dec()
{
...

    {
        ek_t v{100};
        ++v;
        TEST("pre_inc 100->101", v == ek(101));
    }
    {
        ek_t v{100};
        v++;
        TEST("post_inc 100->101", v == ek(101));
    }
    {
        ek_t v{100};
        --v;
        TEST("pre_dec 100->99", v == ek(99));
    }
    {
        ek_t v{100};
        v--;
        TEST("post_dec 100->99", v == ek(99));
    }
    {
        ek_t v{-100};
        ++v;
        TEST("inc neg -100->-99", v == ek(-99));
    }
    {
        ek_t v{-100};
        --v;
        TEST("dec neg -100->-101", v == ek(-101));
    }
    {
        ek_t v{0};
        ++v;
        TEST("inc zero 0->1", v == ek(1));
    }
    {
        ek_t v{0};
        --v;
        TEST("dec zero 0->-1", v == ek(-1));
    }
...
// =============================================================================
// Section 14: EK boundary values — TC_MIN/TC_MAX round-trips
// =============================================================================
static void test_ek_boundaries()
{
    std::cout << "\n--- Section 14: EK boundary values ---\n";

    // EK_MIN: raw stored = (0, 0) => value = 0 - K = -2^126
    // Representable in TC as -2^126 = {0xC000000000000000, 0}
    const ek_t ek_min{0ULL, 0ULL};
    const tc_t tc_of_min{ek_min};
    const ek_t ek_min_back{tc_of_min};
    TEST("EK_MIN->TC->EK round-trip", ek_min_back == ek_min);

    // TC_MAX = 2^127-1: largest TC, within EK range [-2^126, 3*2^126-1]
    const tc_t tc_max{0x7FFFFFFFFFFFFFFFULL, 0xFFFFFFFFFFFFFFFFULL};
    const ek_t ek_max{tc_max};
    const tc_t tc_max_back{ek_max};
    TEST("EK(TC_MAX)->TC round-trip", tc_max_back == tc_max);

    // Ordering: EK_MIN < EK(0) < EK_MAX
    const ek_t ek_zero = ek(0);
    TEST("EK_MIN != EK(0)", ek_min != ek_zero);
    TEST("EK_MAX != EK(0)", ek_max != ek_zero);
    TEST("EK_MIN < EK(0)", ek_min < ek_zero);
    TEST("EK(0) < EK_MAX", ek_zero < ek_max);
    TEST("EK_MIN < EK_MAX", ek_min < ek_max);
    TEST("EK_MIN is_negative", ek_min.is_negative());
    TEST("EK_MAX !is_negative", !ek_max.is_negative());

    // String round-trips for boundary values
    {
        const std::string s = ek_min.to_string();
        const ek_t back = ek_t::from_string(s.c_str());
        TEST("EK_MIN to_string->from_string round-trip", back == ek_min);
    }

    {
        const std::string s = ek_max.to_string();
        const ek_t back = ek_t::from_string(s.c_str());
        TEST("EK_MAX to_string->from_string round-trip", back == ek_max);
    }
}
