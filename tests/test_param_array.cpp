...
static void test_to_array()
{
    ...
    // Small value: low[0]==0x12, high[8]==0x34
    {
        ...
        for (int i = 1; i < 8 && ok; ++i)
            ok = (b[i] == std::byte{0x00});
        for (int i = 9; i < 16 && ok; ++i)
            ok = (b[i] == std::byte{0x00});
...
static void test_from_array()
{
    ...
    // bytes[0]=0xAB bytes[1]=0xCD → low=0xCDAB; bytes[8]=0xEF bytes[9]=0x12 → high=0x12EF
    {
        ...
        b[9] = std::byte{0x12};
        const uint128_t x{b};
        TEST("from_array low=0xCDAB high=0x12EF", x.low() == 0xCDABULL && x.high() == 0x12EFULL);
    }
    ...
    // All 0xFF → low=high=~0
    {
        ...
        for (int i = 0; i < 16; ++i)
            b[i] = std::byte{0xFF};
        const uint128_t x{b};
        TEST("from_array all 0xFF", x.low() == ~0ULL && x.high() == ~0ULL);
    }
}
...
static void test_array_roundtrip()
{
...

    {
        ...
        TEST("array roundtrip arbitrary", back.low() == orig.low() && back.high() == orig.high());
    }
...
static void test_from_bitset()
{
    ...
    // Set bits 0, 63, 64, 127
    {
        ...
        TEST("from_bitset bits 0/63/64/127", x.low() == exp_low && x.high() == exp_high);
    }
...
static void test_bitset_roundtrip()
{
...

    {
        ...
        TEST("bitset roundtrip alternating", back.low() == orig.low() && back.high() == orig.high());
    }
}
