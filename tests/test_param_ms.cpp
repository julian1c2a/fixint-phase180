...
// =============================================================================
// Test: Magnitude-Sign — 133 tests across 14 sections
...
// Section 14 (Fase C3): -0 arithmetic, comparison with ±0 operands
// =============================================================================
...
static void test_inc_dec()
{
...

    {
        ms_t v{1};
        ++v;
        TEST("ms pre_inc +1->+2", v == ms_t{2});
    }
    {
        ms_t v{1};
        v++;
        TEST("ms post_inc +1->+2", v == ms_t{2});
    }
    {
        ms_t v{-1};
        ++v;
        TEST("ms pre_inc -1->0", v.is_zero());
    }
    {
        ms_t v{-1};
        --v;
        TEST("ms pre_dec -1->-2", v == ms_t{-2});
    }
    {
        ms_t v{0};
        ++v;
        TEST("ms inc 0->+1", v == ms_t{1});
    }
    {
        ms_t v{0};
        --v;
        TEST("ms dec 0->-1", v == ms_t{-1});
    }
...
static void test_multiplication_signs()
{
    ...
    // Sign rules via *=
    {
        ms_t a{3}, b{4};
        a *= b;
        TEST("ms *=: 3*4=12 pos", a == ms_t{12} && !a.is_negative());
    }
    {
        ms_t a{3}, b{-4};
        a *= b;
        TEST("ms *=: 3*(-4)=-12", a == ms_t{-12} && a.is_negative());
    }
    {
        ms_t a{-3}, b{4};
        a *= b;
    ...
    // Zero products via *=
    {
        ms_t a{999}, b{0};
        a *= b;
        TEST("ms *=: x*0=0 !neg", a.is_zero() && !a.is_negative());
    }
    {
        ms_t a{0}, b{999};
        a *= b;
        TEST("ms *=: 0*x=0 !neg", a.is_zero() && !a.is_negative());
    }

    // Zero * negative must give +0, not -0
    {
        const ms_t a{0}, b{-100};
        const auto r = a * b;
        TEST("ms *: 0*(-100)=+0", r.is_zero() && !r.is_negative());
    }
    {
        const ms_t a{-100}, b{0};
        const auto r = a * b;
    ...
    // operator* (non-mutating)
    {
        ...
        const auto r = a * b;
        TEST("ms *: 50*100=5000", r == ms_t{5000});
    }
    {
        const ms_t a{-50}, b{100};
        const auto r = a * b;
    ...
    {
        const ms_t a{-1}, b{1000};
        const auto r = a * b;
        TEST("ms *: (-1)*1000=-1000", r == ms_t{-1000});
    }
...
static void test_ms_bitwise()
{
    ...
    {
        const ms_t a{15}, b{7};
        TEST("ms & two positives non-negative", !(a & b).is_negative());
    }
...
static void test_ms_shifts()
{
    ...
    {
        const ms_t x{-5};
        TEST("ms shl neg sign preserved", (x << 1).is_negative());
    }
    // Right shift positive: sign preserved
    {
        const ms_t x{20};
        TEST("ms shr pos sign preserved", !(x >> 1).is_negative());
    }
    // Right shift negative: sign preserved
    {
        const ms_t x{-20};
        TEST("ms shr neg sign preserved", (x >> 1).is_negative());
    }
...
// =============================================================================
// Section 14: -0 arithmetic and comparison with ±0 operands
// =============================================================================
static void test_neg_zero_arithmetic()
{
    std::cout << "\n--- Section 14: MS -0 arithmetic and comparison ---\n";

    ms_t neg_zero{0LL};
    neg_zero.set_high(neg_zero.high() | (1ULL << 63));

    // Comparison: -0 must equal +0 and order between positive and negative values
    TEST("ms (-0) == ms(+0)", neg_zero == ms_t{0LL});
    TEST("ms (+0) == ms(-0)", ms_t{0LL} == neg_zero);
    TEST("ms (-0) < ms(+1)", neg_zero < ms_t{1LL});
    TEST("ms (-0) > ms(-1)", neg_zero > ms_t{-1LL});
    TEST("ms (-0) <= ms(+0)", neg_zero <= ms_t{0LL});
    TEST("ms (-0) >= ms(+0)", neg_zero >= ms_t{0LL});
    TEST("ms (-0) != ms(+1)", neg_zero != ms_t{1LL});
    ...
    // Arithmetic: -0 as left operand (semantic value is zero)
    TEST("ms (-0) + 1 == 1", (neg_zero + ms_t{1LL}) == ms_t{1LL});
    TEST("ms (-0) + (-1) == -1", (neg_zero + ms_t{-1LL}) == ms_t{-1LL});
    TEST("ms (-0) - 1 == -1", (neg_zero - ms_t{1LL}) == ms_t{-1LL});

    // Arithmetic: -0 as right operand
    TEST("ms 1 + (-0) == 1", (ms_t{1LL} + neg_zero) == ms_t{1LL});
    TEST("ms (-1) + (-0) == -1", (ms_t{-1LL} + neg_zero) == ms_t{-1LL});
    TEST("ms 1 - (-0) == 1", (ms_t{1LL} - neg_zero) == ms_t{1LL});

    // Sum of two zeros is zero (sign may vary — use is_zero)
    TEST("ms (-0) + (-0) is_zero", (neg_zero + neg_zero).is_zero());
    TEST("ms (-0) + (+0) is_zero", (neg_zero + ms_t{0LL}).is_zero());
    TEST("ms (+0) + (-0) is_zero", (ms_t{0LL} + neg_zero).is_zero());
    TEST("ms (-0) - (-0) is_zero", (neg_zero - neg_zero).is_zero());
}
...
int main()
{
    ...
    test_string_io();
    test_ms_edge_cases();
    test_neg_zero_arithmetic();

    std::cout << "\n================================================================\n";
    std::cout << "  RESULTS: " << g_passed + g_failed << " run, " << g_passed << " passed, " << g_failed
              << " failed\n";
    std::cout << "================================================================\n";
